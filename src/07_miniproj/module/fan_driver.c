...
static const char* const fan_driver_mode_str[] = {
...
static void fan_timer_frequency_set(fan_driver_data_t* fan_data, int frequency) {
    ...
    int period = 1*HZ/(frequency*2); // multipy freq by two to have half ON and half OFF during one period
...
static void fan_timer_callback(struct timer_list * list) {
    fan_driver_data_t* fan_data = (fan_driver_data_t*) container_of(list, fan_driver_data_t, fan_timer);
    // re-arm the timer
...
static fan_driver_mode_t fan_mode_from_str(const char* mode_str) {
    ...
    for(i=0; i<ARRAY_SIZE(fan_driver_mode_str); i++) {
        if(strncmp(mode_str, fan_driver_mode_str[i], MIN(strlen(mode_str), strlen(fan_driver_mode_str[i]))) == 0) {
...
static void fan_driver_mode_str_set(fan_driver_data_t* fan_data, fan_driver_mode_t mode) {
    mutex_lock(&fan_data->mode_mutex);
    {
        memset(fan_data->mode_str, 0, sizeof(fan_data->mode_str));
        strncpy(fan_data->mode_str, fan_driver_mode_str[mode], MIN(sizeof(fan_data->mode_str), strlen(fan_driver_mode_str[mode])));
    }
    mutex_unlock(&fan_data->mode_mutex);
}

/// @brief change to mode_new and apply the configuration
static void fan_driver_mode_change(fan_driver_data_t* fan_data, fan_driver_mode_t mode_new) {
...
ssize_t sysfs_show(struct device* dev,
                   ...
                   char* buf)
{
    ...
    if(strncmp(TOSTRING(ATTRIBUTE_FREQ), attr->attr.name, sizeof(TOSTRING(ATTRIBUTE_FREQ))) == 0) {
    ...
    } else if(strncmp(TOSTRING(ATTRIBUTE_MODE), attr->attr.name, sizeof(TOSTRING(ATTRIBUTE_MODE))) == 0) {
        sprintf(buf, "%s\n", fan_data->mode_str);
        ret = strlen(buf);
                    ...
                    size_t count)
{
    ...
    if(strncmp(TOSTRING(ATTRIBUTE_FREQ), attr->attr.name, MIN(sizeof(TOSTRING(ATTRIBUTE_FREQ)), strlen(attr->attr.name))) == 0) {
        ...
        if(mode == FAN_MODE_MANUAL) {
            ...
            pr_err("Cannot set frequency while fan_driver is not in \"%s\" mode\n", fan_driver_mode_str[FAN_MODE_MANUAL]);
            ret = -1;

        }


    } else if(strncmp(TOSTRING(ATTRIBUTE_MODE), attr->attr.name, MIN(sizeof(TOSTRING(ATTRIBUTE_MODE)), strlen(attr->attr.name))) == 0) {
...
static int fan_thread(void* data) {
    ...
    while(!kthread_should_stop()) {
        status = wait_event_interruptible(fan_data->queue,
                    (atomic_read(&fan_data->running_flag) == THREAD_RUN_FLAG) || kthread_should_stop());
        PRINT_ON_ERROR(status,"Error while waiting to start thread (%i)", status);
        ...
        // sleep a little bit
        status = wait_event_interruptible_timeout(fan_data->queue, 
                                                  kthread_should_stop(),
                                                  THREAD_SLEEP_SEC*HZ);
...
static int __init fan_driver_init(void) {
    ...
    if (status == 0) status = device_create_file(misc_device.this_device, &CONCATENATE(dev_attr_, ATTRIBUTE_FREQ));
    if (status == 0) status = device_create_file(misc_device.this_device, &CONCATENATE(dev_attr_, ATTRIBUTE_MODE));
    ...
    memset(fan_data->mode_str, 0, sizeof(fan_data->mode_str));
    strncpy(fan_data->mode_str, fan_driver_mode_str[FAN_MODE_AUTO], MIN(sizeof(fan_data->mode_str), sizeof(fan_driver_mode_str[FAN_MODE_AUTO])));
    ...
    if (status == 0) status = gpio_direction_output(fan_data->gpio_nr, fan_data->led_state);
    ...
    fan_data->fan_thread = kthread_run(fan_thread, (void*)fan_data, "fan_driver_thread");
...
static void __exit fan_driver_exit(void) {
    fan_driver_data_t* fan_data = (fan_driver_data_t*)misc_device.this_device->driver_data;
